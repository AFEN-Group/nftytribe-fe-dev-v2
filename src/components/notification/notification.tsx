  ...
  return (
    <div
      ...
      {nots && <div onClick={() => setNots(!nots)} className={style.notification}>
        ...
        {
          !data.length && <div style={{boxShadow:'none', height:'90%',justifyContent:'center',flexDirection:'column',gap:'24px'}}>
           
              <img style={{width:'100px',height:'100px',margin:'0'}} src={require('../Header/not.png')} alt="" />
            <p>No notification right now</p>
          
          </div>
        }
        { data.length&& data?.map((not: any) => (<div className="nots">
