...
const checkIfBIdTimePasses = async (token_id: any, collection_address: any) => {
  await main()
  try {
    ...
    if (new Date() > new Date(parseInt(Bid.closingTime, 10))) {
      if (Bid.highestBidder != '0x0000000000000000000000000000000000000000') {
        return true
      }
      return false
    }
...
const handleAuctionBid = async (
  ...
  try {
    ...
    const bid = await erc721MarketplaceContract.methods
      .bid(token_id, collection_address, nextBidAmount)
      .send({ from: wallet_address, value: nextBidAmount })
