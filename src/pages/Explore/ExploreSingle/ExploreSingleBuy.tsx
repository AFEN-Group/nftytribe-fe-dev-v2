...
const ExploreSingleBuy = () => {
  ...
  const { handleAuctionBid, checkIfBIdTimePasses, collectNft } =
        ...
        try {
          ...
          if (data.on_sale) {
            ...
            const putOffSale = await marketplace_contract.methods
              .putOffSale(data?.collection_address, parseInt(data?.token_id))
  ...
  return (
    <>
      <Header />
      {showBuy && (
        <BuyModal handleClose={handleClose} nft={nft} nftDetails={nftDetails} />
      )}
      {showPutOnSale && (
        <PutOnSaleModal
                    ...
                    <div className={style.prices}>
                      <div className={style.fixedPrices}>
                        ...
                        {nftDetails?.price ? (
                          <p>
                            {Web3.utils.fromWei(
                              parseInt(nftDetails?.price, 10).toString(),
                              "ether"
                            ) || ""}{" "}
                            ETH
                          </p>
