...
const ExploreSingleBuy = () => {
  ...
  const [erc721MarketplaceAddress, setErc721MarketplaceAddress] =
    useState<any>("");
  // erc 1155 addresses
  const [erc1155MintableAddress, setErc1155MintableAddress] = useState<any>("");
  const [erc1155MarketplaceAddress, setErc1155MarketplaceAddress] =
    useState<any>("");
  ...
  const handleSale = async (e: any) => {
    ...
    if (nft.isListed || nft.listingType) {
      await marketPlaceContract.methods
        .putSaleOff(nft.tokenId, collectionAddress)
              ...
              <div className={style.right}>
                        ...
                        <div className={style.dropContent2}>
                          ...
                          <TwitterShareButton
                            ...
                            <div className={style.dropItem}>
                              <p>Twitter</p>
                              <img
                                src={`${dark === "true" ? Twitter2 : Twitter}`}
                                alt="twitter"
                              />
                            </div>
                          </TwitterShareButton>
                ...
                <div className={style.rightTitles}>
                  ...
                  {pathname.includes("item") && (
                    <div className={style.userBx}>
                      <img src={dark === "true" ? User : User2} alt="user" />
                      {nft && <p>{shortenAddress(nft?.ownerOf)}</p>}
                    </div>
                  )}
                  ...
                  <div
                    ...
                    <div className={style.prices}>
                      <div className={style.fixedPrices}>
                        <div className={style.priceGreen}>
                    ...
                    {nft?.amount > 1 && <p>Number of copies : {nft.amount}</p>}
                    {pathname.includes("item") && (
                      <div className={style.Btns}>
                        {nft?.ownerOf !== walletAddress ? (
                          <button
                            ...
                            // }
                            className={`${
                              nft?.listingType ? style.regBtn : style.regBtn2
                            } ${dark === "true" ? "yellowBtn" : "blueBtn"} `}
                            onClick={handleSubmit}>
                            {!nft?.listingType ? "Not On Sale" : "Buy"}
                          </button>
                        ) : (
                          <button
                            // disabled={
                ...
                {tab === "activity" && (
                  <div
                    ...
                    {activities?.length?activities?.map((activity: any, i: any) => (
                    ...
                    )):<p style={{textAlign:'center',marginTop:'20px'}}>No Activities</p>}
                  </div>
                )}
                {tab === "offers" && (
                  <div
...
const DTPopUp = (props: any) => {
  ...
  useEffect(() => {
    if (Verify.Response) props.changeStep(2);
  }, [Verify.Response]);

  const book = UseAxios();
  useEffect(() => {
    if (book.Response) props.changeStep(3);
  }, [book.Response]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (window.ethereum && userWallet) {
      ...
      console.log(physicalMarket, erc721MarketplaceAddress);
      ...
      erc20token = new web3.eth.Contract(
        erc20.abi,
        props.nft.moreInfo.erc20TokenAddress
      );

      if (props?.nft?.amount < 2) {
        try {
          // console.log(erc20token)
          const decimal = await erc20token.methods
            .decimals()
            .call({ from: userWallet });
          // console.log(decimal);
          const amount = deliveryInToken() * 10 ** decimal;
          // console.log(amount);

          console.log(amount);

          await erc20token.methods
            .approve(erc721MarketplaceAddress, `${amount}`)
  ...
  const getTotalPrice = (
    dollarCostPerToken: number,
    nftPrice: number,
    deliveryFeeInUsd: number,
    service = 0.2
  ) => {
    const itemPriceInDollars = nftPrice * dollarCostPerToken;
    const totalFeeUsd = itemPriceInDollars + deliveryFeeInUsd + service;
    const totalFeeInNative = totalFeeUsd / dollarCostPerToken;
    return {
      usd: totalFeeUsd,
      native: totalFeeInNative,
    };
  };
    ...
    <div className="dt_overlay">
        ...
        <motion.div
          ...
          <div className={style.modalTop}>
            ...
            <p className={style.mText}>
              ...
              <strong> {" " + props.nft?.name} </strong>, with a digital twin,
              go to “collected items” to claim item.
            </p>
      ...
      {!completed && (
        <>
          ...
          {props.step < 3 && (
            <form action="">
              <h2>Digital twin delivery information</h2>
              {props.step === 1 && (
                <>
                  <p>Verify your details for recieving the physical item</p>
                  <p
                    style={{
                      fontSize: "10px",
                      color: "red",
                    ...
                    {Verify.error?.data?.message &&
                      Verify.error?.data?.message?.errors[0]}
                  </p>
                ...
                <>
                  ...
                  <div className="inputs">
                    <div className={"fieldBx"}>
                      ...
                      <SelectOption
                        options={rates.Response?.data?.data?.couriers.map(
                          (data: any) => {
                            console.log(data);
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        book.fetchData({
                          ...
                          requestConfig: {
                            request_token:
                              rates.Response?.data?.data?.request_token,
          ...
          {props.step === 3 && (
            <form action="">
              <h2>Digital twin delivery checkout</h2>
              <p>
                Please ,review your delivery mode for the purchase of{" "}
                {props?.nft.name + props.nft.tokenId} digital Item
              </p>

              <div className="checkout">
                <div>
                  <span>Delivery Mode</span>{" "}
                  <span>{onsaleParams?.service}</span>
                </div>
                <div>
                  <span>Delivery Fee</span>
                  <span>${onsaleParams?.rates}</span>
                </div>
                <div>
                  <span>Total Fee</span>
                  <span>
